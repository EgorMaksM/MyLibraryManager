    ...
    , DB(DB)
{
    ...
    gridWidget->setSelectionBehavior(QAbstractItemView::SelectRows);
    ...
    gridWidget->horizontalHeader()->setSectionResizeMode(QHeaderView::ResizeToContents);
    gridWidget->horizontalHeader()->setDefaultAlignment(Qt::AlignLeft | Qt::AlignVCenter);
    gridWidget->horizontalHeader()->setSortIndicatorClearable(true);
...
void TableViewWidget::populateTable(std::vector<Book> books) {
    ...
    for (int i = 0; i < rows; i++) {
        ...
        title->setData(Qt::UserRole, books[i].title);
...
void TableViewWidget::populateTable(std::vector<Author> authors) {
    ...
    for (int i = 0; i < rows; i++) {
        ...
        death->setData(Qt::UserRole, authors[i].death);
...
void TableViewWidget::populateTable(std::vector<User> users) {
    ...
    for (int i = 0; i < rows; i++) {
        ...
        forename->setData(Qt::UserRole, users[i].forename);
        gridWidget->setItem(i, 1, forename);
        QTableWidgetItem* surname = new QTableWidgetItem(users[i].surname);
        surname->setData(Qt::UserRole, users[i].surname);
        gridWidget->setItem(i, 2, surname);
