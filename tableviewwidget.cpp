    ...
    , DB(DB)
{
    ...
    std::vector<User> users = sortUsers(DB, ID);
    populateTable(users);
...
void TableViewWidget::populateTable(std::vector<Book> books) {
    ...
    for (int i = 0; i < rows; i++) {
        ...
        QTableWidgetItem* title = new QTableWidgetItem(QString::fromStdString(books[i].title));
        ...
        if (resultAuthors.size() > 1) {
            ...
            QTableWidgetItem* authors = new QTableWidgetItem(QString::fromStdString(authorsList));
            gridWidget->setItem(i, 3, authors);
        }
        else {
            QString author_str = QString::fromStdString(resultAuthors[0].forename + " " + resultAuthors[0].surname);
...
void TableViewWidget::populateTable(std::vector<Author> authors) {
    ...
    for (int i = 0; i < rows; i++) {
        ...
        gridWidget->setItem(i, 0, id);
        QTableWidgetItem* forename = new QTableWidgetItem(QString::fromStdString(authors[i].forename));
        gridWidget->setItem(i, 1, forename);
        QTableWidgetItem* surname = new QTableWidgetItem(QString::fromStdString(authors[i].surname));
...
void TableViewWidget::populateTable(std::vector<User> users) {
    ...
    for (int i = 0; i < rows; i++) {
        ...
        gridWidget->setItem(i, 0, id);
        QTableWidgetItem* forename = new QTableWidgetItem(QString::fromStdString(users[i].forename));
        gridWidget->setItem(i, 1, forename);
        QTableWidgetItem* surname = new QTableWidgetItem(QString::fromStdString(users[i].surname));
        gridWidget->setItem(i, 2, surname);
        ...
        gridWidget->setItem(i, 3, birth);
        QTableWidgetItem* email = new QTableWidgetItem(QString::fromStdString(users[i].email));
        gridWidget->setItem(i, 4, email);
        QTableWidgetItem* phone = new QTableWidgetItem(QString::fromStdString(users[i].phone));
